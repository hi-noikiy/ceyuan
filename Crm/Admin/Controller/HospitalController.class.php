...
class HospitalController extends CommonController {
	...
	public function index(){
		...
		$hospital = M('Hospital'); 
		$count      = $hospital->count();// 查询满足要求的总记录数
		$Page       = new \Think\Page($count,5);// 实例化分页类 传入总记录数和每页显示的记录数(25)
		$show       = $Page->show();// 分页显示输出
		// 进行分页数据查询 注意limit方法的参数要使用Page类的属性

		$list = $hospital->order('addtime asc')->limit($Page->firstRow.','.$Page->listRows)->select();	
	...
	public function add(){
		...
		if(IS_POST){
			...
			if($hospital->create($data)){
				...
				if($info=$hospital->add()){
					//验证完成创建表
                	if($hospital->createtable($info)){
					$this->success('添加成功,创建表成功√',U('Index'));
					}
					else{
					$this->success('添加成功,但创建数据表失败×',U('Index'));	
						}
					}
				else{
					$this->error('医院添加失败');
	...
	public function del(){	
		$hospital = D('Hospital');
		$id=I('id');
		if($hospital->delete($id)){
			$hospital->deletetable($id);
