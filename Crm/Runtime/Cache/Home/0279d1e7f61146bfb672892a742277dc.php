...
<head>
	...
	.feedBackList div textarea{width: 100%;min-height:2.8rem;line-height: 1.2rem;font-size:0.928rem;color: #353535;background: #fff;}
	...
	.iconsame:after{width: 10px;height: 10px;border:1px solid #c7c7cc;border-left:none;border-bottom: none;transform: rotate(45deg);z-index: 1;content:"";display: block;position: absolute;right: 1rem;top: 1rem;}
...
wx.onMenuShareAppMessage({
 ...
 <div id="state" style="display:none;">1</div>
    ...
    <div id="uid" style="display:none;"><?php echo ($troubleres["id"]); ?></div>
		<li><span>编号</span><div id="uid"><?php echo ($troubleres["order"]); ?></div></li>
		...
		<!--<li class="iconsame"><span>修改时间</span><div><input id="calendars" readonly type="text" placeholder="请选择时间" /></div></li>-->
		<li class="addLiNode">
			<span>修改后图片</span>
			<div>
		...
		<li><span>补充说明</span><div><textarea placeholder="选填" id="remark" name="con"></textarea></div></li>
	</ul>
	<button class="returnList" onClick="zhenggaitijiao()">
		提交
	</button>
	...
	function zhenggaitijiao(){
		...
		if (!isUpload) {
			...
			var url='/Home/Control/zhenggaitijiao';
			...
			data='uid='+uid+'&upimg='+upimg+'&remark='+remark;
	...
	if(mes_obj['state']==0){
		var url='/Home/Control/zhenggaitijiaoaccess/';
		window.location.href=url;
		return;
		}
	if(mes_obj['state']==1 || mes_obj['state']==2 || mes_obj['state']==3){
		alert('上传图片失败,请选择正常的图片');
		}
	if(mes_obj['state']==4){
		alert('创建数据失败');
		}	
